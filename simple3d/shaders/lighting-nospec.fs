...
vec3 CalculateDirLight(DirLight light, vec3 normal, vec3 viewDir) 
{
	...
	vec3 diffuseOut = light.diffuse * diff * Color;

	if (textureSize(material.diffuse, 0).x > 0)
		diffuseOut = vec3(0.0f, 1.0f, 0.0f);
	else
		diffuseOut = light.diffuse * diff * Color;
...
vec3 CalculatePointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir) 
{
	...
	if (textureSize(material.diffuse, 0).x > 0)
		diffuseOut = vec3(1.0f, 0.5f, 0.5f);
	else
		diffuseOut = light.diffuse * diff * Color;
